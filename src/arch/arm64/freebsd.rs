...
impl FreeBsdArm64Probe {
    pub fn query() -> Self {
        ...
        {
            // HWCAP (Auxiliary Vector Type 25) - FreeBSD sys/arm64/include/elf.h
            ...
            // HWCAP2 (Auxiliary Vector Type 26) - FreeBSD sys/arm64/include/elf.h
            ...
            // HWCAP3 (Auxiliary Vector Type 38) - FreeBSD sys/arm64/include/elf.h
            ...
            const AT_HWCAP3: libc::c_int = 38;

            let mut hwcap: libc::c_ulong = 0;
            let mut hwcap2: libc::c_ulong = 0;
            let mut hwcap3: libc::c_ulong = 0;
            ...
            unsafe {
                ...
                let _ = libc::elf_aux_info(
                    AT_HWCAP3,
                    &mut hwcap3 as *mut _ as *mut libc::c_void,
                    std::mem::size_of::<libc::c_ulong>() as libc::c_int,
                );
            }
            ...
            fn check_freebsd_cache() -> bool {
                unsafe {
                    let mut cache_size: libc::c_int = 0;
                    let mut len = std::mem::size_of::<libc::c_int>();
                    let ret = libc::sysctlbyname(
                        c"machdep.cache_line_size".as_ptr(),
                        &mut cache_size as *mut _ as *mut libc::c_void,
                        &mut len,
                        std::ptr::null_mut(),
                        0,
                    );
            ...
            // Cache / Subsystem
            probe.external_cache_available   = check_freebsd_cache();
